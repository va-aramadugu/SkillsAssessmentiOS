...
filter by care type, search by description, restrict to the statement month,
sort by amount, swipe to delete, and record a new charge from the **+** button.
...
go ahead. As a guide, aim to spend roughly the first half of the session on
the reported issues and the second half on structure.
...
4. With **Statement month** on, an urgent care visit from last year is listed.
...
- Feel free to use Xcode's documentation, autocomplete, the simulator, and
  web search. AI coding assistants are not permitted during the session.
- There is no test target. If you would add tests, describe what you would
  test and how.
